...
public class RollingDie extends Die
{
    ...
    private void drawDots(Graphics g, int x, int y, int numDots)
    {
        ...
        int[][] points = {{x1,y1},{x1,y2},{x1,y3},{x3,y1},{x3,y2},{x3,y3},{x2,y2}};
        int[][] combos = {{6},{0,5},{0,6,5},{0,3,2,5},{0,3,2,5,6},{0,1,2,3,4,5}};
        
        for (int i = 0; i < combos[numDots-1].length; i++) {
            int currentX = points[combos[numDots-1][i]][0];
            int currentY = points[combos[numDots-1][i]][1];
            g.fillOval(currentX, currentY, dotSize, dotSize);
        }
    }
}
